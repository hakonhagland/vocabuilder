...
from vocabuilder.vocabuilder import Database, DatabaseException

# from .conftest import database_object, test_data, data_dir_path


def test_additem(caplog: LogCaptureFixture, database_object: Database):
    db = database_object
...
def test_additem_bad(database_object: Database):
    db = database_object
    now = db.epoch_in_seconds()
    item = {
...
def test_additem_bad_type(database_object: Database):
    db = database_object
    now = db.epoch_in_seconds()
    item = {
        db.header.term1: "yes2",
...
def test_datadir(database_object: Database, data_dir_path: PosixPath):
    db = database_object
