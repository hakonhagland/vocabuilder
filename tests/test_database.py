...
class TestBackupRepo:
    def test_create_fail1(
        ...
        filename = data_dir / Database.backup_dirname
    ...
    def test_create_fail2(
        ...
        data_dir = setup_database_dir()
        backupdir = data_dir / Database.backup_dirname
        backupdir.mkdir()
        filename = backupdir / Database.git_dirname
        filename.touch(exist_ok=False)
        cfg = config_object
        voca_name = test_data["vocaname"]
        with pytest.raises(DatabaseException) as excinfo:
...
class TestDatabaseCreate:
    def test_create_fail(
        ...
        data_dir = setup_database_dir()
        filename = data_dir / Database.database_fn
        filename.unlink()
        filename.mkdir()
        cfg = config_object
        voca_name = test_data["vocaname"]
