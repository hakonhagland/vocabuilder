...
@pytest.fixture()
def data_dir_path(tmp_path: PosixPath, test_file_path: PosixPath, test_data: dict) -> PosixPath:
...
@pytest.fixture()
def config_object(config_dir_path: PosixPath, mocker: MockerFixture,
                  data_dir_path: PosixPath) -> Config:
    cfg_dir = config_dir_path
    data_dir = data_dir_path
    mocker.patch('vocabuilder.vocabuilder.platformdirs.user_config_dir', return_value=cfg_dir)
    mocker.patch('vocabuilder.vocabuilder.platformdirs.user_data_dir', return_value=data_dir)
    cfg = Config()
    return cfg

@pytest.fixture()
def database_object(tmp_path: PosixPath, test_file_path: PosixPath, config_object: Config,
                    mocker: MockerFixture, test_data: dict, data_dir_path: PosixPath) -> Database:
    cfg = config_object
    data_dir = data_dir_path
